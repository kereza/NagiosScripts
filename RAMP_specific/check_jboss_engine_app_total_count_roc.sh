...
CURRENT_VAL=`/opt/jboss/bin/twiddle.sh -s $JBOSS_HOST get phase-engine:name=EngineServiceBean,service=EngineServiceBean,type=ManagementInterface ${APP_COUNT_TYPE}s | grep $DESTINATION_NAME | awk -F${DESTINATION_NAME} '{ print $2 }' | awk '{ print $2}' | awk -F, '{print $1}'`
