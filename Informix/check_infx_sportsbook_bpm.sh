...
echo "OK: BPM Source: ${QUEUE} is ${FREEREAL}|${QUEUE}_bpmcount=${FREEREAL};$WARN;$CRIT" 
exit 0
fi

if [ "$WARN" -ge "$FREEREAL" -a "$CRIT" -le "$FREEREAL" ];then
echo "WARN: BPM Source: ${QUEUE} is ${FREEREAL}|${QUEUE}_bpmcount=${FREEREAL};$WARN;$CRIT" 
exit 1
fi

if [ "$CRIT" -ge "$FREEREAL" ];then
echo "CRIT: BPM Source: ${QUEUE} is ${FREEREAL}|${QUEUE}_bpmcount=${FREEREAL};$WARN;$CRIT" 
exit 2
fi
