...
# Get number accepting_conns 
elsif ($o_check =~ /^accepting_conns$/i) {
    my $line = <$SOCKET>;
    while ($line ne "END\r\n") {
        $intData = $1 if ($line =~ m/STAT accepting_conns ([0-9]+)/);  # Get number of evictions
        $line = <$SOCKET>;
    }

    $strOutput    = "$intData number of accepting_conns";
    $strPerfData  = "'accepting_conns'=" . $intData . ";;;;";
}
